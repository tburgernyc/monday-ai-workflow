...
/**
 * BoardService class for managing Monday.com boards
 *
 * This service provides methods to interact with Monday.com boards,
 * including fetching, creating, updating, and deleting boards,
 * as well as managing board activities and items.
 */
export class BoardService {
  private cache: {
    boards?: CacheItem<Board[]>;
    boardsByWorkspace: Map<string, CacheItem<Board[]>>;
    boardById: Map<string, CacheItem<Board>>;
    boardActivity: Map<string, CacheItem<BoardActivity[]>>;
  };
  ...
  constructor() {
    this.cache = {
      boardsByWorkspace: new Map<string, CacheItem<Board[]>>(),
      boardById: new Map<string, CacheItem<Board>>(),
      boardActivity: new Map<string, CacheItem<BoardActivity[]>>()
    };
  }
  ...
  public async getBoards(workspaceId?: string): Promise<Board[]> {
    try {
      ...
      if (workspaceId && this.isCacheValid(this.cache.boardsByWorkspace.get(workspaceId))) {
        MondayLogger.debug(`Using cached boards for workspace ID ${workspaceId}`);
        return this.cache.boardsByWorkspace.get(workspaceId)!.data;
      } else if (!workspaceId && this.isCacheValid(this.cache.boards)) {
        MondayLogger.debug('Using cached boards');
        return this.cache.boards!.data;
      }
      ...
      // Update cache
      if (workspaceId) {
        this.cache.boardsByWorkspace.set(workspaceId, {
          data: boards,
          timestamp: Date.now()
        });
      } else {
  ...
  public async getBoardById(id: string): Promise<Board | null> {
    try {
      // Check cache first
      const cachedBoard = this.cache.boardById.get(id);
      if (this.isCacheValid(cachedBoard)) {
        MondayLogger.debug(`Using cached board for ID ${id}`);
        return cachedBoard!.data;
      }
  ...
  public async createBoard(
    ...
    try {
      ...
      // Invalidate caches
      delete this.cache.boards;
      if (workspaceId) {
        this.cache.boardsByWorkspace.delete(workspaceId);
      }
  ...
  public async duplicateBoard(id: string, name: string): Promise<Board> {
    try {
      ...
      if (originalBoard?.workspace_id) {
        this.cache.boardsByWorkspace.delete(originalBoard.workspace_id);
      }
  ...
  public async updateBoard(
    ...
    try {
      ...
      this.cache.boardById.delete(id); // Delete instead of update as we might not have all fields
      delete this.cache.boards;
      
      // Get the board to check its workspace
      const fullBoard = await this.getBoardById(id);
      if (fullBoard?.workspace_id) {
        this.cache.boardsByWorkspace.delete(fullBoard.workspace_id);
      }
  ...
  public async deleteBoard(id: string): Promise<{ id: string }> {
    try {
      ...
      // Invalidate caches
      this.cache.boardById.delete(id);
      delete this.cache.boards;
      this.cache.boardActivity.delete(id);
      
      if (board?.workspace_id) {
        this.cache.boardsByWorkspace.delete(board.workspace_id);
      }
  ...
  public async getBoardActivity(id: string, limit: number = 50): Promise<BoardActivity[]> {
    try {
      // Check cache first
      const cacheKey = id;
      const cachedActivity = this.cache.boardActivity.get(cacheKey);
      if (this.isCacheValid(cachedActivity)) {
      ...
      // Update cache
      this.cache.boardActivity.set(cacheKey, {
        data: activities,
        timestamp: Date.now()
      });
  ...
  public async getBoardItems(
    ...
    try {
      ...
      MondayLogger.debug(`Fetching items for board ID ${boardId}`, options);
  ...
  public async createGroup(boardId: string, groupName: string): Promise<Group> {
    try {
      ...
      this.cache.boardById.delete(boardId);
  ...
  public clearCache(): void {
    delete this.cache.boards;
    this.cache.boardsByWorkspace.clear();
    this.cache.boardById.clear();
    this.cache.boardActivity.clear();
